...
public class RedissonLockController {
	...
	public void tryLock() {
		...
		if (lockFlag) {
			...
			try {
				...
				// 释放锁
				lock.unlock();
				MySlf4j.textInfo("业务执行完成，释放锁");
			}
		} else {
			MySlf4j.textError("获取锁失败");
		}
	...
	public void lock() {
		...
		RLock lock = redissonClient.getLock("myLock");
		try {
			// 默认枷锁时间是 30s，没有最大等待时间，会一直等待直到锁释放，另一个线程会再次获取锁
			lock.lock();
			MySlf4j.textInfo("获取锁成功, 执行任务!");
			Thread.sleep(5000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		} finally {
